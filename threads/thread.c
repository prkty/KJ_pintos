...
thread_unblock (struct thread *t) {
	...
	// 내림 차순으로 정리
	list_insert_ordered(&ready_list, &t->elem, priority_fix, NULL);
...
bool priority_fix (const struct list_elem *a, const struct list_elem *b, void *aux UNUSED) {
	...
	if(thread_a == NULL || thread_b == NULL) return false;
...
thread_yield (void) {
	...
	old_level = intr_disable ();
	if (curr != idle_thread)
		list_insert_ordered(&ready_list, &curr->elem, priority_fix, NULL);
...
void check_priority_Max(void) {
	...
	if(thread_get_priority() < t->priority && !intr_context ()) thread_yield();
}
...
init_thread (struct thread *t, const char *name, int priority) {
	...
	/* Project 1 Priority donation Part */
...
void remove_with_lock(struct lock *lock) {
	...
	while(curr != list_end(&t->donations)) {
		...
		if(curr_thread->wait_lock == lock) list_remove(&curr_thread->donation_elem);
